...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self._features_sem = torch.empty(0)  # [ADD]
    ...
    @property
    def get_features_sem(self):  # [ADD]
        return self._features_sem
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._features_sem = nn.Parameter(torch.zeros((fused_point_cloud.shape[0], 16)).float().cuda().requires_grad_(True))  # [ADD]
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._features_sem], 'lr': training_args.feature_lr, "name": "features_sem"},  # [ADD]
    ...
    def prune_points(self, mask):
        ...
        self._opacity = optimizable_tensors["opacity"]
        self._features_sem = optimizable_tensors["features_sem"]  # [ADD]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_features, new_opacities, new_scaling, new_rotation, new_depth_err, new_features_sem=None):  # [ADD]
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "features": new_features,
        "features_sem": new_features_sem if new_features_sem is not None else torch.zeros((new_xyz.shape[0], self._features_sem.shape[1]), device=new_xyz.device),  # [ADD]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, opa_thresh, N=2):
        ...
        new_features_sem = self._features_sem[selected_pts_mask].repeat(N,1)  # [ADD]
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        new_depth_err = self._depth_err[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_features, new_opacity, new_scaling, new_rotation, new_depth_err, new_features_sem)  # [ADD]
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, opa_thresh):
        ...
        new_features_sem = self._features_sem[selected_pts_mask]  # [ADD]
        new_opacities = self._opacity[selected_pts_mask]
        new_depth_err = self._depth_err[selected_pts_mask]
        new_scaling = self._scaling[selected_pts_mask]
        new_rotation = self._rotation[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_features, new_opacities, new_scaling, new_rotation, new_depth_err, new_features_sem)  # [ADD]
