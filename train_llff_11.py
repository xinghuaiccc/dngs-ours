...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, near_range):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # [ADD] VCGC update: view-conditioned instability collapse
        if getattr(opt, "vcgc_enable", False) and iteration >= getattr(opt, "vcgc_start_iter", 800) and iteration % getattr(opt, "vcgc_period", 200) == 0:
            with torch.no_grad():
                train_views = scene.getTrainCameras().copy()
                if len(train_views) > 0:
                    num_views = min(getattr(opt, "vcgc_views_per_update", 3), len(train_views))
                    sampled_views = random.sample(train_views, num_views)
                    contrib_views = []
                    for vcam in sampled_views:
                        vcgc_pkg = render(vcam, gaussians, pipe, background, return_contrib=True)
                        contrib = vcgc_pkg.get("contrib", None)
                        if contrib is not None and contrib.numel() > 0:
                            contrib_views.append(contrib)
                    if len(contrib_views) > 0:
                        contrib_stack = torch.stack(contrib_views, dim=0)
                        mean_c = contrib_stack.mean(dim=0)
                        var_c = (contrib_stack - mean_c).pow(2).mean(dim=0)
                        instability = var_c / (mean_c.pow(2) + 1e-6)
                        k = max(1, int(getattr(opt, "vcgc_topq", 0.10) * instability.numel()))
                        k = min(k, instability.numel())
                        top_vals, top_idx = torch.topk(instability, k, largest=True)
                        strength = max(getattr(opt, "vcgc_strength", 1.0), 1e-6)
                        # Collapse scaling to isotropy and clamp
                        scaling = getattr(gaussians, "_scaling", None)
                        if scaling is not None:
                            selected = scaling[top_idx]
                            mean_log = selected.mean(dim=1, keepdim=True)
                            iso_target = max(getattr(opt, "vcgc_iso_target", 1.0), 1e-6)  # [ADD]
                            selected = mean_log + torch.log(torch.tensor(iso_target, device=selected.device))  # [ADD]
                            selected = selected.repeat(1, selected.shape[1])
                            cap = getattr(opt, "vcgc_scale_cap", 0.02) / strength
                            log_cap = torch.log(torch.tensor(max(cap, 1e-9), device=selected.device))
                            selected = torch.minimum(selected, log_cap)
                            scaling.data[top_idx] = selected
                        # Optional opacity damping
                        opacity_param = getattr(gaussians, "_opacity", None)
                        if opacity_param is not None:
                            opacity_param.data[top_idx] -= strength
                        # Optional rotation freeze via mask
                        if getattr(opt, "vcgc_freeze_rot", True):
                            rotation = getattr(gaussians, "_rotation", None)
                            if rotation is not None:
                                rot_mask = torch.zeros((rotation.shape[0],), dtype=torch.bool, device=rotation.device)
                                rot_mask[top_idx] = True
                                gaussians.vcgc_rot_mask = rot_mask
                        else:
                            gaussians.vcgc_rot_mask = None  # [ADD]
                        if getattr(opt, "vcgc_merge", False):
                            print("[VCGC] merge requested but not implemented")  # [ADD]
                        if iteration % getattr(opt, "vcgc_log_every", 200) == 0:
                            mean_i = instability.mean().item()
                            top_i = top_vals[0].item() if top_vals.numel() > 0 else 0.0
                            mean_scale = torch.exp(scaling[top_idx]).mean().item() if scaling is not None else 0.0
                            mean_opa = torch.sigmoid(opacity_param[top_idx]).mean().item() if opacity_param is not None else 0.0
                            print(f"[VCGC] iter={iteration} meanI={mean_i:.6f} topI={top_i:.6f} |S|={k} mean_scale={mean_scale:.6f} mean_opacity={mean_opa:.6f}")
        ...
        if gfalo_active:
            ...
            if appearance_model is not None:
                for param in appearance_model.parameters():
                    if param.grad is not None:
                        param.grad.zero_()
        # [ADD] VCGC rotation freeze mask
        vcgc_rot_mask = getattr(gaussians, "vcgc_rot_mask", None)
        if vcgc_rot_mask is not None:
            rotation = getattr(gaussians, "_rotation", None)
            if rotation is not None and getattr(rotation, "grad", None) is not None:
                rotation.grad[vcgc_rot_mask] = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vcgc_enable", action="store_true", default=False)  # [ADD]
    parser.add_argument("--vcgc_start_iter", type=int, default=800)  # [ADD]
    parser.add_argument("--vcgc_period", type=int, default=200)  # [ADD]
    parser.add_argument("--vcgc_views_per_update", type=int, default=3)  # [ADD]
    parser.add_argument("--vcgc_topq", type=float, default=0.10)  # [ADD]
    parser.add_argument("--vcgc_strength", type=float, default=1.0)  # [ADD]
    parser.add_argument("--vcgc_iso_target", type=float, default=1.0)  # [ADD]
    parser.add_argument("--vcgc_scale_cap", type=float, default=0.02)  # [ADD]
    parser.add_argument("--vcgc_freeze_rot", action="store_true", default=True)  # [ADD]
    parser.add_argument("--no_vcgc_freeze_rot", dest="vcgc_freeze_rot", action="store_false")  # [ADD]
    parser.add_argument("--vcgc_merge", action="store_true", default=False)  # [ADD]
    parser.add_argument("--vcgc_log_every", type=int, default=200)  # [ADD]
    ...
    opt.vcgc_enable = getattr(args, "vcgc_enable", False)  # [ADD]
    opt.vcgc_start_iter = getattr(args, "vcgc_start_iter", 800)  # [ADD]
    opt.vcgc_period = getattr(args, "vcgc_period", 200)  # [ADD]
    opt.vcgc_views_per_update = getattr(args, "vcgc_views_per_update", 3)  # [ADD]
    opt.vcgc_topq = getattr(args, "vcgc_topq", 0.10)  # [ADD]
    opt.vcgc_strength = getattr(args, "vcgc_strength", 1.0)  # [ADD]
    opt.vcgc_iso_target = getattr(args, "vcgc_iso_target", 1.0)  # [ADD]
    opt.vcgc_scale_cap = getattr(args, "vcgc_scale_cap", 0.02)  # [ADD]
    opt.vcgc_freeze_rot = getattr(args, "vcgc_freeze_rot", True)  # [ADD]
    opt.vcgc_merge = getattr(args, "vcgc_merge", False)  # [ADD]
    opt.vcgc_log_every = getattr(args, "vcgc_log_every", 200)  # [ADD]
