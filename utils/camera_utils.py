...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    # [ADD] optional DINO feature map
    dino_feat = None
    if cam_info.dino_feat is not None:
        dino_arr = cam_info.dino_feat
        if dino_arr.ndim == 3:
            if dino_arr.shape[0] in (3, 16):
                dino_tensor = torch.from_numpy(dino_arr).float()
            else:
                dino_tensor = torch.from_numpy(np.transpose(dino_arr, (2, 0, 1))).float()
        elif dino_arr.ndim == 2:
            dino_tensor = torch.from_numpy(dino_arr[None, ...]).float()
        else:
            dino_tensor = torch.from_numpy(np.array(dino_arr)).float()
        dino_tensor = dino_tensor.unsqueeze(0)
        dino_tensor = F.interpolate(dino_tensor, size=(resolution[1], resolution[0]), mode='bilinear', align_corners=False)
        dino_feat = dino_tensor.squeeze(0)

    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, 
                  image=gt_image, gt_alpha_mask=loaded_mask, depth_mono=resized_depth_mono, dino_feat=dino_feat,
