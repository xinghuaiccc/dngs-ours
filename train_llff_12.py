...
# [ADD] SDS helper using diffusers (local weights only)
class SDSHelper:
    def __init__(self, model_path, device="cuda"):
        try:
            ...
            raise RuntimeError("diffusers/transformers not available; install locally to use SDS") from exc
        if not model_path:
        ...
        self.text_encoder = CLIPTextModel.from_pretrained(model_path, subfolder="text_encoder", local_files_only=True).to(device)
        self.vae = AutoencoderKL.from_pretrained(model_path, subfolder="vae", local_files_only=True).to(device)
        self.unet = UNet2DConditionModel.from_pretrained(model_path, subfolder="unet", local_files_only=True).to(device)
        self.scheduler = DDIMScheduler.from_pretrained(model_path, subfolder="scheduler", local_files_only=True)
    ...
    def sds_loss(self, image, sds_resolution=512):
        img = image.unsqueeze(0)
        img = torch.nn.functional.interpolate(img, (sds_resolution, sds_resolution), mode="bilinear", align_corners=False)
        img = img.clamp(0.0, 1.0)
        img = img * 2.0 - 1.0
        with torch.cuda.amp.autocast(enabled=True):  # [ADD]
            latents = self.vae.encode(img).latent_dist.rsample()
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=True):  # [ADD]
            eps_pred = self.unet(latents_noisy, t, encoder_hidden_states=self.empty_prompt_emb).sample
        alpha = self.scheduler.alphas_cumprod[t].to(latents.device)
        while alpha.ndim < latents.ndim:
            alpha = alpha.view(-1, 1, 1, 1)
        weight = (1.0 - alpha)
        grad = weight * (eps_pred - noise)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, near_range):
    ...
    if getattr(opt, "sds_enable", False):
        sds_helper = SDSHelper(getattr(opt, "sds_model_path", ""), device="cuda")  # [ADD]
    if checkpoint:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        loss += loss_reg
        # [ADD] SDS loss (after start iter with warmup)
        if sds_helper is not None and iteration > getattr(opt, "sds_start_iter", 1500):
            ...
            loss_sds = sds_helper.sds_loss(image, sds_resolution=getattr(opt, "sds_resolution", 512))
            if iteration % 200 == 0:
                print(f"[SDS] iter={iteration} loss_sds={loss_sds.item():.6f} w_sds={w_sds:.6f}")  # [ADD]
            loss += w_sds * loss_sds
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sds_enable", action="store_true", default=False)  # [ADD]
    parser.add_argument("--sds_model_path", type=str, default="")  # [ADD]
    parser.add_argument("--sds_start_iter", type=int, default=1500)  # [ADD]
    parser.add_argument("--sds_warmup", type=int, default=1000)  # [ADD]
    parser.add_argument("--sds_weight", type=float, default=0.1)  # [ADD]
    parser.add_argument("--sds_resolution", type=int, default=512)  # [ADD]
    ...
    opt = op.extract(args)
    # [ADD] propagate SDS params into optimization args
    opt.sds_enable = getattr(args, "sds_enable", False)
    opt.sds_model_path = getattr(args, "sds_model_path", "")
    opt.sds_start_iter = getattr(args, "sds_start_iter", 1500)
    opt.sds_warmup = getattr(args, "sds_warmup", 1000)
    opt.sds_weight = getattr(args, "sds_weight", 0.1)
